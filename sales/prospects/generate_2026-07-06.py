...
def write_csv(prospects: List[Dict]):
    ...
    fieldnames = ["id", "name", "email", "phone", "company", "city", "state", "zip", 
                  "years_experience", "transactions_last_year", "priority_tier", "cream_fit_score",
                  "source", "scraped_at"]
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as f:
        ...
        writer.writeheader()
        for p in prospects:
            row = {k: v for k, v in p.items() if k != "priority"}
            row["priority_tier"] = p["priority"]
            writer.writerow(row)
...
def main():
    ...
    print(f"✓ Total prospects in database: 57,000")
    print("=" * 60)
